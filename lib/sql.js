...
module.exports = {
  ...
  readRowByValue: function (object) {
    return new Promise((resolve, reject) => {
      ...
      if (!(
        'condition' in object && object.condition &&
        object.condition.constructor === {}.constructor &&
        'key' in object.condition && object.condition.key &&
        'value' in object.condition && object.condition.value
      )) {
        // Invalid data
      ...
      self.db.all(
        `SELECT * FROM ${object.name}
        WHERE ${object.condition.key} = "${object.condition.value}"`,
        (err, data) => {
          if (err) {
            reject(err)
          }
